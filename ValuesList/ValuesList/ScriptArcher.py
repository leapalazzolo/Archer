...
def unir_archivos(parametros_de_entrada, archivo_salida):
    ...
    lista_archivos = []
    for carpeta, subcarpeta, archivos in os.walk(os.path.join(parametros_de_entrada, '')):
        ...
        with open(archivo, 'r') as infile:
            for line in infile:
                if cortar_linea.search(line):
                    line = re.sub('</xs:complexType><xs:simpleType', r'</xs:complexType>\n<xs:simpleType', line)
...
def main():
    ayuda = """
                ...
                Se creara un archivo con el resumen de la comparación de lista de valores llamado 'Comparacion.csv' con el resultado del analisis.
                Por ultimo, se informara un resumen de la ejecucion por pantalla al finalizar.
                """
    parser = optparse.OptionParser(usage=ayuda, version="%prog Alpha 1.2")
    parser.add_option('-c', '--compare', dest='compare', help='Compara 2 salidas del script (Archivos de salida de la ejecucion con la opcion "-p").', action='store_true')
    parser.add_option('-m', '--merge', dest='merge', help='Une 2 o mas archivos de entrada XSD.', action='store_true')
    parser.add_option('-p', '--parse', dest='parse', help='Realiza el parseo del XSD de entrada.', action='store_true')
    parser.add_option('-i', '--input', dest='input', help='Ingrese la carpeta donde se encuentran los archivo XML obtenidos antes de realizar la migracion.', action='store_true')
    parser.add_option('-o', '--output', dest='output', help='Ingrese la carpeta donde se encuentran los archivo XML obtenidos luego de realizar la migracion.', action='store_true')
    (options, args) = parser.parse_args()
    if options.input and options.output:
        with open('Primer_archivo.xsd.out', 'w+') as primer_archivo_xsd:
            unir_archivos(sys.argv[2], primer_archivo_xsd)
            primer_archivo_xsd.close()
            with open('Segundo_archivo.xsd.out', 'w+') as segundo_archivo_xsd:
                unir_archivos(sys.argv[4], segundo_archivo_xsd)
                segundo_archivo_xsd.close()
                with open('Primer_archivo_datos.out','w+') as primer_archivo_xsd_datos:
                    primer_archivo_xsd = open(primer_archivo_xsd.name, 'r')
                    parsear_archivo(primer_archivo_xsd, primer_archivo_xsd_datos)
                    with open('Segundo_archivo_datos.out', 'w+') as segundo_archivo_xsd_datos:                    
                        segundo_archivo_xsd = open(segundo_archivo_xsd.name, 'r')
                        ...
                        with open('Comparacion_ListasdeValores.csv','w+') as archivo_comparacion:
                            comparar_archivos(primer_archivo_xsd_datos, segundo_archivo_xsd_datos.name, archivo_comparacion)
      ...
      #              parsear_archivo(archivo_entrada,archivo_salida)
        ...
        if len(sys.argv) == 4 and options.compare:
            with open('Comparacion.csv', 'w+') as archivo_comparacion:
                ...
                comparar_archivos(f1, sys.argv[3], archivo_comparacion)
        #else:
         #   print 'Modos de uso:\n' \
          #      '1. Ingrese la opcion "-p" y la ruta del archivo de Archer para parsearlo. Salida esperada: Archivo.out\n' \
           #     '2. Ingrese la opcion "-m" y 2 o mas rutas de archivos de Archer para fusionarlos o 3 o más rutas. Salida esperada: Archivo.out\n' \
            #    '3. Ingrese la opcion "-c" y 2 rutas de archivos procesados por este script para comparar diferencias. Salida esperada: Comparacion.csv'
...
def comparar_archivos(datos_primer_archivo, ruta_datos_segundo_archivo, outfile):
    ...
    # with open('Comparacion.csv','w+') as outfile:
    outfile.write('"Diferencia";"Aplicación.Nombre del campo";"Número de valores"\n')
    for line1 in datos_primer_archivo:
        if line1 == '"Aplicacion.Nombre";"GUID";"Valor/Tipo;Ponderación valor"\n':
            campo1, valores = [], []
        else:
                ...
                if line1 != '\n':#and line1.split(';') >= 3:
                    ...
                    if line1 == '\n':
                        ...
                        with open(ruta_datos_segundo_archivo, 'r') as datos_segundo_archivo:
                            ...
                            while encontrado == False and line2:
                                if line2 == '"Aplicacion.Nombre";"GUID";"Valor/Tipo;Ponderación valor"\n':
                                    campo2, valores = [], []
                                else:
                                            ...
                                            if line2 == '\n':
                                                ...
                                                if campo1[1] == campo2[1]:
                                                    if len(campo1[2]) == len(campo2[2]):
                                                        ...
                                                        campos_distintos += 1
                                                        outfile.write('"Cambio de valores";' + campo1[0] + ';De ' + str(len(campo1[2])) + ' campos a ' + str(len(campo2[2])) + ' campos.;\n')
                                                    encontrado = True
                                line2 = datos_segundo_archivo.readline()
                            if encontrado == False:
...
def extaer_campo(line):
    cantidad_valores = 0
    campo, valores = [] , []
    if line == '"Aplicacion.Nombre";"GUID";"Valor/Tipo;Ponderación valor"\n':
        cantidad_valores = 0
    else:
...
def parsear_archivo(archivo_entrada,archivo_salida):
    ...
    obtener_nombre_campos = re.compile(r'</archer:ValuesListValueDefinition>.*?</xs:simpleType><xs:complexType '
                                       'name=(".*?").*'
                                        #'archer:Id=("[\d]*") '
    ...
    '''obtener_campos_calculados = re.compile(r'</archer:CalculationFormula>.*?</xs:complexType><xs:simpleType.*? '
                                             'name=(".*?").*'
                                             r'archer:Id=("[\d]*") '
                                             'archer:Guid=("[A-Za-z0-9_-]*").*?'
                                             'archer:Type=("ValuesList")'
        ...
        if len(line) < 10000:#and linea_valida is not None: #and linea_valida is not None:
            ...
            if campo:
                ...
                if flag_imprimir_en_archivo is True:
                    cantidad_campos += 1
                    imprimir_en_archivo(datos_campo[0], '"Aplicacion.Nombre";"GUID";"Valor/Tipo;Ponderación valor"', valores, archivo_salida)
                ...
                if lista_de_valores:
                    ...
                    if lista_de_valores:
                        ...
                        if campo_nombre:
                            ...
                            cantidad_campos += 1
                            imprimir_en_archivo(tupla, '"Aplicacion.Nombre";"GUID";"Valor/Tipo;Ponderación valor"', valores,archivo_salida)
